...
export default {
  ...
  data() {
    return { sewageOutfalls: [], drawerVisible: false }
  },
  async mounted() {
        ...
        for (let i = 0, len = this.sewageOutfalls.length; i < len; i++) {
          ...
          if (item.longitude && item.latitude) {
            ...
            graphic.on(window.$ZMap.EventType.tooltipopen, async (e) => {
              const { code, data } = await apiData.getSewageOutfallInfo({ id: e.target.attr.id })
...
<template>
  <div>
    <DrawerSewageOutfall :visible="drawerVisible" @close="drawerVisible = false" />
  </div>
</template>
