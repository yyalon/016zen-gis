...
import PopupReservoir from '../popup/Reservoir.vue'
...
export default {
  ...
  data() {
    return { reservoirs: [], drawerVisible: false, drawerData: {} }
  },
  async mounted() {
  ...
  methods: {
    ...
    async showLayer() {
      ...
      if (_layer) {
        ...
        for (let i = 0, len = this.reservoirs.length; i < len; i++) {
          const item = this.reservoirs[i]
          if (item.JD && item.WD) {
